...
**Q:** You say A2A is too heavy for Gemma, but the A2A shim is pure Python stdlib — no extra dependencies. The HTTP server only runs when explicitly started. How is this heavier than running tmux or the [rbitr]?
...
**Q:** You say [rbitr] can route by capability, but how does [rbitr] know a worker's capabilities? Agent Cards aren't consumed by [rbitr] today.

**A:** Today rbitr uses a static target map. Agent Cards would replace that. Each worker registers its Card on spawn. [rbitr] reads Cards from `.well-known/agent.json` to build a dynamic routing table. This is a separate task but enables exactly what we need.
...
**A:** Cards are written once per session, not per task. The I/O cost is negligible. The benefit is canonical identity: no more guessing runtime from env vars. [rbitr] can route dynamically. Other agents can discover capabilities without probing.
...
**A:** Content negotiation adds complexity to every request. Separate ports make it obvious which protocol you're speaking. Also, A2A and rbitr may run in different processes (A2A shim is lightweight, rbitr has SQLite + event log). Separate ports = separate failure domains.

### [2026-05-17T00:59Z] rbitr → neo | claim:rbitr-should-be-a2a-native | round:5

**Q:** You want [rbitr] fully A2A-native, but that means rewriting the dispatch envelope, spawn logic, and event model. What's the incremental path?
...
2. **bundled-is-correct** — All agree. rbitr routes by capability.
